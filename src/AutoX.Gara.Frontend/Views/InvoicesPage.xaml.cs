...
public partial class InvoicesPage : ContentPage
{
    ...
    private async void OnTaxRateTapped(Object? sender, TappedEventArgs e)
        => await PickerActionSheetHelper.ShowAsync(sender as VisualElement, "Thuế", _vm.TaxRateOptions, idx => _vm.PickerTaxRateIndex = idx);

    private async void OnDiscountTypeTapped(Object? sender, TappedEventArgs e)
        => await PickerActionSheetHelper.ShowAsync(sender as VisualElement, "Loại giảm giá", _vm.DiscountTypeOptions, idx => _vm.PickerDiscountTypeIndex = idx);

    private async void OnPaymentStatusTapped(Object? sender, TappedEventArgs e)
        => await PickerActionSheetHelper.ShowAsync(sender as VisualElement, "Trạng thái thanh toán", _vm.PaymentStatusOptions, idx => _vm.PickerPaymentStatusIndex = idx);
