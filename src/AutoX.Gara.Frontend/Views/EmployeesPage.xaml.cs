...

{
    ...
    private void OnPositionFilterTapped(object? sender, TappedEventArgs e)
        ...
        => ShowFilterFlyout(sender, "Ch?n ch?c v?", vm => vm.FilterPositionOptions, (vm, idx) => vm.PickerPositionIndex = idx, vm => vm.PickPositionCommand);

    private void OnStatusFilterTapped(object? sender, TappedEventArgs e)

        => ShowFilterFlyout(sender, "Ch?n tr?ng th�i", vm => vm.FilterStatusOptions, (vm, idx) => vm.PickerStatusIndex = idx, vm => vm.PickStatusCommand);

    private void OnGenderFilterTapped(object? sender, TappedEventArgs e)

        => ShowFilterFlyout(sender, "Ch?n gi?i t�nh", vm => vm.FilterGenderOptions, (vm, idx) => vm.PickerGenderIndex = idx, vm => vm.PickGenderCommand);

    private void OnSalaryFilterTapped(object? sender, TappedEventArgs e)

        => ShowFilterFlyout(sender, "L?c theo luong", vm => vm.SalaryFilterOptions, (vm, idx) => vm.PickerSalaryIndex = idx, vm => vm.PickSalaryCommand);
