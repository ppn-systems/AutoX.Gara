...

{
...

    {
...

        {
            ...
            if (r is Directive err) return EmployeeSalaryListResult.Failure(err.Reason.ToString(), err.Action);

            return EmployeeSalaryListResult.Failure("Unknown response", ProtocolAdvice.NONE);
...

    {
...

        {
            ...
            if (echo && r is EmployeeSalaryDto confirmed) return EmployeeSalaryWriteResult.Success(confirmed);
