...
///   <item>Cache hit hoàn toàn bypass network — tr? v? ngay t? memory.</item>
/// </list>
/// </para>
/// </summary>
public sealed class CustomerService : ICustomerService
{
    ...
    {
        // -- Cache hit: tr? v? ngay, không t?n bang thông -----------------
        ...
        System.Threading.CancellationToken ct = default)
    {
        ...
        // D? li?u m?i ? cache cu không còn chính xác
        if (result.IsSuccess)
        {
            _cache.Invalidate();
        }
    ...
    private static System.String MapErrorReason(ProtocolReason reason)
        => reason switch
        {
            ...
            ProtocolReason.ALREADY_EXISTS => "Email ho?c s? di?n tho?i dã t?n t?i.",
            ...
            ProtocolReason.RATE_LIMITED => "B?n dang thao tác quá nhanh. Vui lòng ch? m?t chút r?i th? l?i.",
            ProtocolReason.UNSUPPORTED_PACKET => "Yêu c?u không du?c h? tr?. Vui lòng c?p nh?t ph?n m?m n?u có th?.",
            ProtocolReason.CRYPTO_UNSUPPORTED => "L?i mã hóa. Vui lòng c?p nh?t ph?n m?m n?u có th?.",
            ProtocolReason.COMPRESSION_FAILED => "L?i nén d? li?u. Vui lòng c?p nh?t ph?n m?m n?u có th?.",
            ProtocolReason.TRANSFORM_FAILED => "L?i x? lý d? li?u. Vui lòng c?p nh?t ph?n m?m n?u có th?.",
            ProtocolReason.TIMEOUT => "Máy ch? ph?n h?i h?t h?n. Vui lòng th? l?i.",
            _ => "Thao tác th?t b?i. Vui lòng th? l?i."
        };
