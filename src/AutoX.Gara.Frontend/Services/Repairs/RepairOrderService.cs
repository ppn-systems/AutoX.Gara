...

{
...

    {
...

        {
            ...
            if (r is Directive err) return RepairOrderListResult.Failure(err.Reason.ToString(), err.Action);

            return RepairOrderListResult.Failure("Unknown response", ProtocolAdvice.NONE);
...

    {
...

        {
            ...
            if (echo && r is RepairOrderDto confirmed) return RepairOrderWriteResult.Success(confirmed);
