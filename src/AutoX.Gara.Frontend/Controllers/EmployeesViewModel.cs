...
public sealed partial class EmployeesViewModel : ObservableObject, System.IDisposable
{
    ...
    public String[] FilterStatusOptions { get; } =
        StatusValues.Select((v, idx) => idx == 0 ? "T?t c? tr?ng thái" : EnumText.Get(v)).ToArray();
    ...
    public System.String FormTitle => IsEditing ? "S?a nhân viên" : "Thêm nhân viên";
    public System.String FormSaveText => IsEditing ? "Luu thay d?i" : "Thêm nhân viên";
    ...
    private async System.Threading.Tasks.Task ConfirmChangeStatusAsync()
    {
        ...
        try
        {
            ...
            {
                HandleError("Ð?i tr?ng thái th?t b?i", result.ErrorMessage!, result.Advice);
            }
        }
        finally
        {
    ...
    private System.Boolean ValidateForm()
    {
        ...
        { SetFormError("Ngày sinh ph?i trong quá kh?."); return false; }

        if (FormEndDate.HasValue && FormEndDate <= FormStartDate)
        { SetFormError("Ngày k?t thúc ph?i sau ngày b?t d?u."); return false; }
