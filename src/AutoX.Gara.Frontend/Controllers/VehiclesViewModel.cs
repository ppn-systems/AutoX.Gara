...
/// <summary>
/// ViewModel cho VehiclesPage — hi?n th? danh sách xe + form thêm/s?a/xóa
...
public sealed partial class VehiclesViewModel : ObservableObject, System.IDisposable
{
    ...
    public System.String FormTitle => IsEditing ? "S?a xe" : "Thêm xe m?i";
    public System.String FormSaveText => IsEditing ? "Luu thay d?i" : "Thêm xe";
    ...
    public string[] FormBrandOptions { get; } =
        BrandValues.Select((v, idx) => idx == 0 ? "— ch?n —" : EnumText.Get(v)).ToArray();
    public string[] FormTypeOptions { get; } =
        TypeValues.Select((v, idx) => idx == 0 ? "— ch?n —" : EnumText.Get(v)).ToArray();
    public string[] FormColorOptions { get; } =
        ColorValues.Select((v, idx) => idx == 0 ? "— ch?n —" : EnumText.Get(v)).ToArray();
