...
public sealed partial class CustomersViewModel : ObservableObject, System.IDisposable
{
    ...
    // Gi? nguyên FilterType/FilterMembership d? logic không d?i
    ...
    public string[] FormTypeOptions { get; } =
        CustomerTypeValues.Select((v, idx) => idx == 0 ? "— ch?n —" : EnumText.Get(v)).ToArray();

    public string[] FormMembershipOptions { get; } =
        MembershipValues.Select((v, idx) => idx == 0 ? "— ch?n —" : EnumText.Get(v)).ToArray();

    public string[] FormGenderOptions { get; } =
        GenderValues.Select((v, idx) => idx == 0 ? "— ch?n —" : EnumText.Get(v)).ToArray();

    // FIX PICKER: Picker.SelectedIndex (int) thay vì SelectedItem (string?enum không match)
    // 0=T?t c?, 1=Cá nhân, 2=Doanh nghi?p
    ...
    public System.String FormTitle => IsEditing ? "S?a khách hàng" : "Thêm khách hàng";
    public System.String FormSaveText => IsEditing ? "Luu thay d?i" : "Thêm khách hàng";
    ...
    private async System.Threading.Tasks.Task LoadAsync()
    {
        ...
        try
        {
            ...
            {
                HandleWriteError("T?i danh sách th?t b?i", result.ErrorMessage!, result.Advice);
            }
        }
        finally
        {
    ...
    {
        // T?o page m?i, truy?n customer context vào tru?c khi navigate
    ...
    private async System.Threading.Tasks.Task ConfirmDeleteAsync()
    {
        ...
        try
        {
            ...
            {
                HandleWriteError("Xóa th?t b?i", result.ErrorMessage!, result.Advice);
            }
        }
        finally
        {
    ...
    private System.Boolean ValidateForm()
    {
        ...
        { SetFormError("Mã s? thu? b?t bu?c d?i v?i khách hàng doanh nghi?p."); return false; }
