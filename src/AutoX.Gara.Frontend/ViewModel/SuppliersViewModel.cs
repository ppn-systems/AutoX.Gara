...
public sealed partial class SuppliersViewModel : ObservableObject, System.IDisposable
{
    ...
    public string[] FilterStatusOptions { get; } =
        new[] { UiTextConfiguration.Current.SuppliersFilterStatusAllText }
            .Concat(StatusValues.Where(v => v != SupplierStatus.None).Select(EnumText.Get))
            .ToArray();
    public string[] FilterPaymentTermsOptions { get; } =
        new[] { UiTextConfiguration.Current.SuppliersFilterPaymentTermsAllText }
            .Concat(PaymentTermsValues.Where(v => v != PaymentTerms.None).Select(EnumText.Get))
            .ToArray();
    ...
    public string FormTitle => IsEditing
        ? UiTextConfiguration.Current.SuppliersFormTitleEditText
        : UiTextConfiguration.Current.SuppliersFormTitleCreateText;
    public string FormSaveText => IsEditing
        ? UiTextConfiguration.Current.CommonFormSaveChangesText
        : UiTextConfiguration.Current.SuppliersFormSaveCreateText;
    ...
    private async System.Threading.Tasks.Task LoadAsync()
    {
        ...
        try
        {
            ...
            {
                HandleError(UiTextConfiguration.Current.SuppliersErrorLoadFailedText, result.ErrorMessage!, result.Advice);
            }
        }
        finally
        {
    ...
    private async System.Threading.Tasks.Task ConfirmChangeStatusAsync()
    {
        ...
        try
        {
            ...
            {
                HandleError(UiTextConfiguration.Current.SuppliersErrorChangeStatusFailedText, result.ErrorMessage!, result.Advice);
            }
        }
        finally
        {
    ...
    private bool ValidateForm()
    {
        ...
        { SetFormError(UiTextConfiguration.Current.SuppliersValidationNameRequiredText); return false; }
        if (FormName.Length > 150)
        { SetFormError(UiTextConfiguration.Current.SuppliersValidationNameMaxLengthText); return false; }
        if (!IsValidEmail(FormEmail))
        { SetFormError(UiTextConfiguration.Current.SuppliersValidationEmailInvalidText); return false; }
        if (string.IsNullOrWhiteSpace(FormTaxCode))
        { SetFormError(UiTextConfiguration.Current.SuppliersValidationTaxCodeRequiredText); return false; }
        if (FormPhoneNumbers.Length > 0)
        {
            foreach (var phone in FormPhoneNumbers.Split(','))
            {
                if (!IsValidPhoneNumber(phone.Trim()))
                { SetFormError(UiTextConfiguration.Current.SuppliersValidationPhoneInvalidText); return false; }
            }
        }
        if (FormContractEndDate.HasValue && FormContractEndDate <= FormContractStartDate)
        { SetFormError(UiTextConfiguration.Current.SuppliersValidationEndDateAfterStartDateText); return false; }
        return true;
    }
