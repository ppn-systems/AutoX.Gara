...
public sealed partial class ServiceItemsViewModel : ObservableObject, System.IDisposable
{
    ...
    public string FormTitle => IsEditing
        ? UiTextConfiguration.Current.ServiceItemsFormTitleEditText
        : UiTextConfiguration.Current.ServiceItemsFormTitleCreateText;
    public string FormSaveText => IsEditing
        ? UiTextConfiguration.Current.CommonFormSaveChangesText
        : UiTextConfiguration.Current.ServiceItemsFormSaveCreateText;
    ...
    public ServiceItemsViewModel(ServiceItemService service)
    {
        ...
        ServiceTypeFilterOptions = new[] { UiTextConfiguration.Current.ServiceItemsFilterTypeAllText }
            .Concat(all.Select(EnumText.Get))
            .ToArray();
        _serviceTypeFormValues = all;
        ServiceTypeFormOptions = new[] { UiTextConfiguration.Current.ServiceItemsFormTypePlaceholderText }
            .Concat(all.Select(EnumText.Get))
            .ToArray();
    ...
    private async System.Threading.Tasks.Task LoadAsync()
    {
        ...
        try
        {
            ...
            {
                HandleError(UiTextConfiguration.Current.ServiceItemsErrorLoadFailedText, result.ErrorMessage!, result.Advice);
            }
        }
        finally
        {
    ...
    private async System.Threading.Tasks.Task ConfirmDeleteAsync()
    {
        ...
        try
        {
            ...
            {
                HandleError(UiTextConfiguration.Current.ServiceItemsErrorDeleteFailedText, result.ErrorMessage!, result.Advice);
            }
        }
        finally
        {
    ...
    private bool ValidateForm()
    {
        ...
        { SetFormError(UiTextConfiguration.Current.ServiceItemsValidationDescriptionRequiredText); return false; }
        if (FormDescription.Length > 255)
        { SetFormError(UiTextConfiguration.Current.ServiceItemsValidationDescriptionMaxLengthText); return false; }
        if (!decimal.TryParse(FormUnitPrice, out decimal price) || price <= 0)
        { SetFormError(UiTextConfiguration.Current.ServiceItemsValidationUnitPricePositiveText); return false; }
        if (FormTypeIndex == 0)
        { SetFormError(UiTextConfiguration.Current.ServiceItemsValidationTypeRequiredText); return false; }
        return true;
    }
