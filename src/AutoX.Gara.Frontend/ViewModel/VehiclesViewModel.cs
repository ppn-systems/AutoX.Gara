...

{
    ...
    public string PageTitle => Owner is not null ? $"Xe c?a {Owner.Name}" : "Danh s�ch xe";
    ...
    public string FormTitle => IsEditing ? "S?a xe" : "Th�m xe m?i";

    public string FormSaveText => IsEditing ? "Luu thay d?i" : "Th�m xe";
    ...
    public string[] FormTypeOptions { get; } = [.. TypeValues.Select((v, idx) => idx == 0 ? "� ch?n �" : EnumText.Get(v))];

    public string[] FormBrandOptions { get; } = [.. BrandValues.Select((v, idx) => idx == 0 ? "� ch?n �" : EnumText.Get(v))];

    public string[] FormColorOptions { get; } = [.. ColorValues.Select((v, idx) => idx == 0 ? "� ch?n �" : EnumText.Get(v))];
...

    {
        ...
        { SetFormError("Bi?n s? xe kh�ng được d? tr?ng."); return false; }

        if (FormLicensePlate.Length > 20)

        { SetFormError("Bi?n s? xe kh�ng được vượt qu� 20 k� t?."); return false; }

        if (FormYear < 1900 || FormYear > DateTime.Now.Year + 1)

        { SetFormError($"Nam s?n xu?t kh�ng h?p l? (1900 � {DateTime.Now.Year + 1})."); return false; }

        if (FormMileage < 0)

        { SetFormError("S? km kh�ng được �m."); return false; }
