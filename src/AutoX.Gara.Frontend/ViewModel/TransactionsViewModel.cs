...

{
    ...
    public string PageTitle => Invoice?.InvoiceNumber is null ? "Thanh to�n" : $"Thanh to�n {Invoice.InvoiceNumber}";
...

    {
...

        {
            ...
            {
                HandleError("Kh�ng t?i được giao d?ch", result.ErrorMessage ?? "Thao t�c th?t b?i.", result.Advice);
...

    {
...

        {
            ...
            {
                HandleError("T?o giao d?ch th?t b?i", result.ErrorMessage ?? "Thao t�c th?t b?i.", result.Advice);
