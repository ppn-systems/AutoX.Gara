...
public sealed partial class RepairOrderItemsViewModel : ObservableObject, System.IDisposable
{
    ...
    private readonly Dictionary<int, string> _partNameById = [];
    private List<RepairOrderItemDto> _rawItems = [];
    ...
    public string PageTitle => RepairOrder?.RepairOrderId is null
        ? UiTextConfiguration.Current.RepairOrderItemsPageTitleText
        : string.Format(System.Globalization.CultureInfo.CurrentCulture, UiTextConfiguration.Current.RepairOrderItemsPageTitleWithIdText, RepairOrder.RepairOrderId);
    ...
    private void RefreshPartFilter()
    {
        ...
        string term = PartSearchTerm?.Trim() ?? string.Empty;
    ...
    {
        ItemRows.Clear();
        for (int i = 0; i < _rawItems.Count; i++)
        {
            RepairOrderItemDto it = _rawItems[i];
            _partNameById.TryGetValue(it.PartId, out string? name);
    ...
    public async System.Threading.Tasks.Task LoadLookupsAsync()
    {
        ...
        try
        {
            ...
            {
                HasError = true;
                ErrorMessage = string.Format(System.Globalization.CultureInfo.CurrentCulture, UiTextConfiguration.Current.RepairOrderItemsErrorLoadPartsText, result.ErrorMessage ?? UiTextConfiguration.Current.CommonErrorActionFailedText);
                return;
            }
            PartOptions.Clear();
            _partNameById.Clear();
            for (int i = 0; i < result.Parts.Count; i++)
            {
                ...
                {
                    string display = $"#{p.PartId.Value} - {p.PartName} ({p.SellingPrice:n0})";
        ...
        catch (Exception ex)
        {
            LogException(ex);
            HasError = true;
            ErrorMessage = string.Format(System.Globalization.CultureInfo.CurrentCulture, UiTextConfiguration.Current.RepairOrderItemsErrorLoadPartsText, ex.Message);
        }
        finally
        {
    ...
    public async System.Threading.Tasks.Task LoadAsync()
    {
        ...
        try
        {
            ...
            {
                HasError = true;
                ErrorMessage = result.ErrorMessage ?? UiTextConfiguration.Current.CommonErrorActionFailedText;
                return;
            }
    ...
    private async System.Threading.Tasks.Task SaveAsync()
    {
        ...
        {
            HasError = true;
            ErrorMessage = UiTextConfiguration.Current.RepairOrderItemsValidationPartAndQuantityText;
            return;
        }
        IsLoading = true;
        ClearError();
