...

{
    ...
    public string PageTitle => RepairOrder?.RepairOrderId is null
        ...
        ? UiText.Get("repair_order_items.page_title", "Part trong lệnh")

        : UiText.Format("repair_order_items.page_title_with_id", "Part lệnh #{0}", RepairOrder.RepairOrderId);
...

    {
...

        {
...

            {
                ...
                ErrorMessage = UiText.Format("repair_order_items.error.load_parts", "Không tải được danh sách part: {0}", result.ErrorMessage ?? UiText.Get("common.error.action_failed", "Thao tác thất bại."));
...

        {
            ...
            ErrorMessage = UiText.Format("repair_order_items.error.load_parts", "Không tải được danh sách part: {0}", ex.Message);
...

    {
...

        {
...

            {
                ...
                ErrorMessage = result.ErrorMessage ?? UiText.Get("common.error.action_failed", "Thao tác thất bại.");
...

    {
...

        {
            ...
            ErrorMessage = UiText.Get("repair_order_items.validation.part_and_quantity", "Vui lòng chọn Part và nhập Quantity > 0.");
