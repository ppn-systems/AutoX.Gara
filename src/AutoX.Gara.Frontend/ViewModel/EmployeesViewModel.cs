...
public sealed partial class EmployeesViewModel : ObservableObject, System.IDisposable
{
    ...
    public String[] FilterPositionOptions { get; } = [.. FilterPositionValues.Select((v, idx) => idx == 0 ? UiTextConfiguration.Current.EmployeesFilterPositionAllText : EnumText.Get(v))];
    public String[] FilterStatusOptions { get; } = [.. StatusValues.Select((v, idx) => idx == 0 ? UiTextConfiguration.Current.EmployeesFilterStatusAllText : EnumText.Get(v))];
    public String[] FilterGenderOptions { get; } = [.. GenderValues.Select((v, idx) => idx == 0 ? UiTextConfiguration.Current.CommonFilterAllText : EnumText.Get(v))];
    ...
    [
        UiTextConfiguration.Current.EmployeesSalaryFilterAllText,
        UiTextConfiguration.Current.EmployeesSalaryFilterHasSalaryText,
        UiTextConfiguration.Current.EmployeesSalaryFilterNoSalaryText,
        UiTextConfiguration.Current.EmployeesSalaryFilterMonthlyText,
        UiTextConfiguration.Current.EmployeesSalaryFilterDailyText,
        UiTextConfiguration.Current.EmployeesSalaryFilterHourlyText,
    ];
    ...
    public string FormTitle =>
        IsEditing
            ? UiTextConfiguration.Current.EmployeesFormTitleEditText
            : UiTextConfiguration.Current.EmployeesFormTitleCreateText;
    public string FormSaveText =>
        IsEditing
            ? UiTextConfiguration.Current.CommonFormSaveChangesText
            : UiTextConfiguration.Current.EmployeesFormSaveCreateText;
    ...
    public sealed class EmployeeRow : ObservableObject
    {
        ...
        private string _salaryText = UiTextConfiguration.Current.EmployeesSalaryNoneText;
    ...
    public String SalaryFormTitle =>
        IsSalaryEditing
            ? UiTextConfiguration.Current.EmployeesSalaryFormTitleEditText
            : UiTextConfiguration.Current.EmployeesSalaryFormTitleCreateText;
        ...
        try
        {
            ...
            if (result.IsSuccess)
            {
                ...
                foreach (EmployeeDto e in result.Employees)
                {
                    ...
                    {
                        SalaryText = UiTextConfiguration.Current.EmployeesSalaryNoneText,
                        HasSalary = false,
            ...
            {
                HandleError(UiTextConfiguration.Current.EmployeesErrorLoadFailedText, result.ErrorMessage!, result.Advice);
            }
        }
        finally
        {
    ...
    private async System.Threading.Tasks.Task PickPositionAsync()
    {
        ...
        String pick = await page.DisplayActionSheetAsync(
            UiTextConfiguration.Current.EmployeesPickerFilterPositionText,
            cancelText,
            null,
    ...
    private async System.Threading.Tasks.Task PickStatusAsync()
    {
        ...
        string cancelText = UiTextConfiguration.Current.CommonActionCancelText;
        String pick = await page.DisplayActionSheetAsync(
            UiTextConfiguration.Current.EmployeesPickerFilterStatusText,
            cancelText,
            null,
            FilterStatusOptions);
        if (pick == cancelText || String.IsNullOrWhiteSpace(pick))
    ...
    private async System.Threading.Tasks.Task PickGenderAsync()
    {
        ...
        string cancelText = UiTextConfiguration.Current.CommonActionCancelText;
        String pick = await page.DisplayActionSheetAsync(
            UiTextConfiguration.Current.EmployeesPickerFilterGenderText,
            cancelText,
            null,
            FilterGenderOptions);
        if (pick == cancelText || String.IsNullOrWhiteSpace(pick))
    ...
    private async System.Threading.Tasks.Task PickSalaryAsync()
    {
        ...
        string cancelText = UiTextConfiguration.Current.CommonActionCancelText;
        String pick = await page.DisplayActionSheetAsync(
            UiTextConfiguration.Current.EmployeesPickerFilterSalaryText,
            cancelText,
            null,
            SalaryFilterOptions);
        if (pick == cancelText || String.IsNullOrWhiteSpace(pick))
    ...
    private async System.Threading.Tasks.Task ConfirmChangeStatusAsync()
    {
        ...
        try
        {
            ...
            {
                HandleError(UiTextConfiguration.Current.EmployeesErrorChangeStatusFailedText, result.ErrorMessage!, result.Advice);
            }
        }
        finally
        {
    ...
    private async System.Threading.Tasks.Task SaveSalaryFormAsync()
    {
        ...
        {
            SetSalaryFormError(UiTextConfiguration.Current.EmployeesSalaryValidationEmployeeRequiredText);
            return;
        }
        if (SalaryFormSalary < 0)
        {
            SetSalaryFormError(UiTextConfiguration.Current.EmployeesSalaryValidationAmountNonNegativeText);
            return;
        }
        if (SalaryFormUnit < 0)
        {
            SetSalaryFormError(UiTextConfiguration.Current.EmployeesSalaryValidationUnitNonNegativeText);
            return;
        }
        ...
        {
            await Microsoft.Maui.ApplicationModel.MainThread.InvokeOnMainThreadAsync(() =>
                SetSalaryFormError(result.ErrorMessage ?? UiTextConfiguration.Current.EmployeesSalaryErrorSaveFailedText));
            return;
        }
        // Update row display.
        ...
        {
            String text = saved is null ? UiTextConfiguration.Current.EmployeesSalaryNoneText : FormatSalaryText(saved);
    ...
    private async System.Threading.Tasks.Task WarmupSalarySummariesAsync(List<EmployeeRow> rows)
    {
        ...
        for (int i = 0; i < rows.Count; i++)
        {
            ...
            String text = latest is null ? UiTextConfiguration.Current.EmployeesSalaryNoneText : FormatSalaryText(latest);
    ...
    private bool ValidateForm()
    {
        ...
        { SetFormError(UiTextConfiguration.Current.EmployeesValidationNameInvalidText); return false; }
        if (!AccountValidation.IsValidEmail(FormEmail))
        { SetFormError(UiTextConfiguration.Current.EmployeesValidationEmailInvalidText); return false; }
        if (!string.IsNullOrWhiteSpace(FormPhoneNumber))
        {
            if (!AccountValidation.IsValidVietnamPhoneNumber(FormPhoneNumber))
            { SetFormError(UiTextConfiguration.Current.EmployeesValidationPhoneInvalidText); return false; }
        }
        if (!EmployeeValidation.IsValidDateOfBirth(FormDateOfBirth))
        { SetFormError(UiTextConfiguration.Current.EmployeesValidationDobInPastText); return false; }
        if (!EmployeeValidation.IsValidDates(FormStartDate, FormEndDate))
        { SetFormError(UiTextConfiguration.Current.EmployeesValidationEndAfterStartText); return false; }
        return true;
    }
