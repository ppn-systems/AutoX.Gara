...
public sealed class EmployeeSalaryRepository(AutoXDbContext context) : IEmployeeSalaryRepository
{
    ...
    public async System.Threading.Tasks.Task<(List<EmployeeSalary> Items, int TotalCount)> GetPageAsync(
        ...
        System.Threading.CancellationToken ct = default)
    {
        ...
        {
            q = q.Where(es => (es.EffectiveTo ?? DateTime.MaxValue) <= query.FilterToDate.Value);
        }
        ...
        List<EmployeeSalary> items = await q
